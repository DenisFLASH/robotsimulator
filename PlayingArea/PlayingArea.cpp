...
void PlayingArea::createWorld()
{
    ...
    /* MOBILE OBJECTS*/
    Robot* teamAMainRobot = new Robot("Ryad", TEAM_A_MAIN_ROBOT_INIT_X, TEAM_A_MAIN_ROBOT_INIT_Y,
                                     TEAM_A_MAIN_ROBOT_RADIUS, EAST);
    p_allMobileObjects.push_back(teamAMainRobot);



    cout << "World created (" << p_allFixedObjects.size() << " fixed objects, "
         << p_allMobileObjects.size() << " mobile objects).\n" << endl;
}
