...
Scene::Scene(PlayingArea* area) : QGraphicsScene()
{
    ...
    for(unsigned int idx = 0; idx < fixedObjects.size(); idx++)
    {
        ...
        scaleCoordinatesOfFixedObject(obj, &xScaled, &yScaled, &widthScaled, &heightScaled);
        obj->showObjectInfo();
...
void Scene::scaleCoordinatesOfFixedObject(FixedObject* obj, int* xScaled, int* yScaled, int* widthScaled, int* heightScaled)
{
    ...
    *widthScaled = width * scaleFactorX;
    *heightScaled = height * scaleFactorY;
}
